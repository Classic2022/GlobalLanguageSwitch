<?php
// Form handler for Urban Reparaturen contact form using PHPMailer
    ...
    if (empty($name) || empty($email) || empty($message)) {
        echo json_encode([
            ...
            'message' => (strpos($_SERVER['HTTP_ACCEPT_LANGUAGE'], 'de') !== false) ? 
                'Name, Email und Nachricht sind Pflichtfelder.' : 
...
";
    ...
    $plain_text = "Neue Kontaktanfrage von der Urban Reparaturen Webseite:
...
$message";
    ...
    try {
        ...
        // SMTP Configuration - Update these with your Hostinger SMTP settings
        $mail->isSMTP();
        $mail->Host       = 'smtp.hostinger.com'; // Replace with your Hostinger SMTP server
        $mail->SMTPAuth   = true;
        $mail->Username   = 'info@urban-r.de'; // Replace with your Hostinger email
        ...
        // Sender & recipients
        $mail->setFrom('info@urban-r.de', 'Urban Reparaturen Website');
        $mail->addAddress($to);
